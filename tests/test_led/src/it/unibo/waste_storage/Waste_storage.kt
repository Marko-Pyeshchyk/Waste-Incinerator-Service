		...
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					...
					sysaction { //it:State
					 ...
					 transition(edgeName="t122",targetState="handle_data",cond=whenEvent("scale_data"))
				}	 
				state("handle_data") { //this:State
					action { //it:State
						if( checkMsgContent( Term.createTerm("scale_data(N)"), Term.createTerm("scale_data(W)"), 
								 ...
								 var W = payloadArg(0)  
								CommUtils.outyellow("$name	weight: $W")
								forward("weight", "weight($W)" ,"wis" ) 
						}
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
					sysaction { //it:State
					 ...
					 transition(edgeName="t123",targetState="handle_data",cond=whenEvent("scale_data"))
				}	 
			}
		}
} 
